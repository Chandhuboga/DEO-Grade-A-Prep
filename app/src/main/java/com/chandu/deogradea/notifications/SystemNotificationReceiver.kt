...
class SystemNotificationReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val day = DeoGradeAPlan.currentDay(LocalDate.now()).coerceIn(1, 95)
        ...
        when (action) {
            SystemNotificationScheduler.ACTION_DANGER -> {
                if (done < 7) {
                    NotificationHelper.showDanger(context, day, 7 - done)
            ...
            SystemNotificationScheduler.ACTION_INACTIVITY -> {
                ...
                if (done < 7 && daytime && now - lastProgress >= 4L * 60L * 60L * 1000L && now - lastWarning >= 4L * 60L * 60L * 1000L) {
            ...
            SystemNotificationScheduler.ACTION_DAILY_QUEST_EVENING -> {
                ...
                val summary = "${quest.mainQuest} • ${quest.phase} • $done/7 objectives done"
